...
| (void *arg) | The argument passed in from pthread_create. This is a void* so it can point to anything—int, struct, array, etc. You just cast it inside the function. | 
```
3 ) pthread_join():
It’s used by one thread (usually main) to wait for another thread to finish execution. This ensures the program doesn’t terminate before all threads are done.
```c
...
| void **retval | A pointer to a pointer where the return value from the thread function (i.e., what it returns with return) will be stored. Use NULL if you don’t care about the return value. | 
```
 Return Value
- Returns 0 on success.
