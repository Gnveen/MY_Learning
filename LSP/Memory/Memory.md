...
```
```
86. Demonstrate dynamic memmory allocaiton using mallloc()
```c
#include <stdio.h>
#include <stdlib.h>
...
int main() {
    int *ptr;
    ptr = (int *)calloc(4,sizeof(4)); // allocate / updating memory for 4 integers
...
int isIn(int f[], int size, int p) {
    for (int i = 0; i < size; i++) if (f[i] == p) return 1;
    return 0;
}
int findLRU(int t[], int size) {
    int min = t[0], pos = 0;
    for (int i = 1; i < size; i++)
        if (t[i] < min) { min = t[i]; pos = i; }
    return pos;
}
int predict(int p[], int n, int f[], int size, int i) {
    int pos = -1, far = i;
    for (int j = 0; j < size; j++) {
        int k;
        for (k = i; k < n; k++)
...
int main() {
    int pages[100], frame[10], time[10], n, fsize, ch, i, j, pos = 0, count = 0, faults = 0;
    printf("Pages: "); scanf("%d", &n);
    printf("Page numbers: "); for (i = 0; i < n; i++) scanf("%d", &pages[i]);
    printf("Frames: "); scanf("%d", &fsize);
    printf("1.FIFO  2.LRU  3.Optimal\nChoose: "); scanf("%d", &ch);
    for (i = 0; i < fsize; i++) frame[i] = -1;
    for (i = 0; i < n; i++) {
        if (!isIn(frame, fsize, pages[i])) {
            if (ch == 1) { // FIFO
                frame[pos] = pages[i]; pos = (pos + 1) % fsize;
            }
            else if (ch == 2) { // LRU
                ...
                if (count < fsize) frame[count++] = pages[i];
                else frame[predict(pages, n, frame, fsize, i + 1)] = pages[i];
            }
            faults++;
        }
        for (j = 0; j < fsize; j++) printf("%d ", frame[j]);
        printf("\n");
        if (ch == 2)
            for (j = 0; j < fsize; j++)
